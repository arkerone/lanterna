...
import { selfHotspotUserCaller } from '../shared/attribution.js';
import {
  allocatorUserCaller,
  buildMemoryFinding,
  isActionableMemoryFrame,
} from '../shared/memory-finding.js';
...
export const allocInHotPathDetector: KindScopedDetector<'cpu' | 'memory'> = {
  ...
  detect({ cpu, memory }): Finding[] {
    ...
    const cpuHotspots = cpu.report.hotspots.filter(isActionableMemoryFrame);
    const actionableMemAllocators = memory.report.hotAllocators.filter(isActionableMemoryFrame);
...
function systemCalleeCpuPct(hotspot: Hotspot, hotspotById: ReadonlyMap<string, Hotspot>): number {
  return hotspot.callees.reduce((total, callee) => {
    ...
    if (!calleeHotspot || isActionableMemoryFrame(calleeHotspot)) return total;
...
function attributedSystemCpuPct(allHotspots: readonly Hotspot[], userHotspot: Hotspot): number {
  return allHotspots.reduce((total, hotspot) => {
    if (isActionableMemoryFrame(hotspot)) return total;
...
function isDominantSystemAllocator(allocator: MemoryHotAllocator): boolean {
  return !isActionableMemoryFrame(allocator) && allocator.totalPct >= 40;
}
