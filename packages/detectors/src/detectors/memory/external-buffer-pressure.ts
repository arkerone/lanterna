...
import { findActionableUserCpuHotspot } from '../shared/attribution.js';
import {
  type CorrelatedAllocatorEvidence,
  correlatedAllocatorFromCpuHotspot,
  correlatedAllocatorFromMemory,
} from '../shared/memory-evidence.js';
import { buildMemoryFinding } from '../shared/memory-finding.js';
...
export const externalBufferPressureDetector: KindScopedDetector<'memory'> = {
  ...
  detect({ memory }, shared): Finding[] {
    ...
    const bufferPressureFinding: BaseFinding<string, Record<string, unknown>> = buildMemoryFinding({
      id: 'external-buffer-pressure',
      severity,
