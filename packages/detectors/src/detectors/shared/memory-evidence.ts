...
import { isEditableMemoryAllocator, isRuntimeAllocatorPath } from './memory-actionability.js';
...
function selectCorrelatedMemoryAllocator(
  ...
  if (matchingHotAllocator && isEditableMemoryAllocator(matchingHotAllocator)) {
...
function findEditableAllocatorForEvidence(
  ...
  return (
    hotAllocators.find(
      (allocator) => isEditableMemoryAllocator(allocator) && allocator.file === preferredFile,
    ) ?? hotAllocators.find(isEditableMemoryAllocator)
  );
}
