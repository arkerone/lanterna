...
export type AttributedFindingCategory = Extract<
  BuiltinFindingCategory,
  | 'blocking-io'
  | 'sync-crypto'
  | 'json-on-hot-path'
...
export type AttributedFindingExtraFor<C extends AttributedFindingCategory> = C extends 'blocking-io'
  ? BlockingIoEvidenceExtra
 ...
 */
export function buildAttributedFinding<C extends AttributedFindingCategory>(options: {
  id: string;
  category: C;
...
}): BaseFinding<C, AttributedFindingExtraFor<C>> {
  const {
    id,
    category,
    severity,
  ...
  return {
    ...
    evidence: {
      ...
      extra: extra as AttributedFindingExtraFor<C>,
    },
    measurements,
    remediation,
    why,
