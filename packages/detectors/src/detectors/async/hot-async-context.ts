...
export const hotAsyncContextDetector: KindScopedDetector<'cpu' | 'async'> = {
  ...
  detect({ async }): Finding[] {
    ...
    return collectAsyncListFindings(attribution.topChains, thresholds.maxFindings, (entry) => {
      if (entry.cpuPct < thresholds.minCpuPct) return stopAsyncList; // sorted desc
      ...
      return emitAsyncFinding({
        ...
        anchor,
        frame,
        fallback: { file: '(async)', line: 0, function: `chain#${entry.rootAsyncId}` },
        userCaller,
        id: `hot-async-context:${entry.rootAsyncId}`,
        severity,
        category: 'hot-async-context',
        title: titleFor(entry),
        confidence: confidenceForAsyncFinding(async.report, { base: 'high' }),
