...
import { anchorForFrame, resolveAsyncUserCaller } from '../shared/async-evidence.js';
import {
...
export const longAwaitDetector: KindScopedDetector<'async'> = {
  ...
  detect({ async }): Finding[] {
    ...
    const dropped = hasAsyncRecordLoss(report);
    return collectAsyncListFindings(
      rankLongAwaitOperations(report.topOperations),
      thresholds.maxFindings,
      (op) => {
        ...
        if (op.latencyCause === 'background') return skipAsyncItem;
        if (isBackgroundWindowOperation(op, captureDurationMs)) return skipAsyncItem;
        if (isUnattributedRootHandle(op)) return skipAsyncItem;
        if (isShadowedByResumedOperation(op, report.topOperations)) return skipAsyncItem;
        if (op.durationMs < thresholds.minDurationMs) return stopAsyncList; // sorted desc
        ...
        const confidence = confidenceForAsyncFinding(report, { base: baseConfidence });
        return emitAsyncFinding({
          report,
          anchor,
          frame,
          fallback: { file: '(async)', line: 0, function: `${op.kind}#${op.asyncId}` },
          userCaller,
