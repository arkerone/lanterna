  ...
  return {
    ...
    nodeOptions:
      instrumentationMode === 'full'
        ...
        : undefined,
    source: `const selectAsyncRecordEvictionCandidate = ${selectAsyncRecordEvictionCandidate.toString()};(${installAsyncOperations.toString()})(__lanterna, ${maxRecords}, ${concurrencyIntervalMs}, ${includeMicrotasks ? 'true' : 'false'}, ${stackDepth}, ${maxRunWindows}, ${JSON.stringify(instrumentationMode)}, ${attachPartialCapture ? 'true' : 'false'}, ${JSON.stringify(cjsAwaitTransformRuntimeSource)});`,
  };
}

export function selectAsyncRecordEvictionCandidate(
  records: Map<number, { durationMs?: number }>,
  completedRecords: Set<number>,
  scanLimit: number,
): number | undefined {
  ...
  const hook = asyncHooks.createHook({
    init(asyncId: number, type: string, triggerAsyncId: number) {
      ...
      if (records.size >= maxRecords) {
        ...
        const evictId = selectAsyncRecordEvictionCandidate(
          records,
          completedRecords,
          EVICTION_SCAN_LIMIT,
        );
